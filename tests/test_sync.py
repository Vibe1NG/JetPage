...
def test_sync_git_docs_cleans_up_stale_repos(tmp_path):
    from jetpage.core.nav import Document, GitSource
    from jetpage.core.sync import sync_git_docs
    import jetpage.core.sync
    ...
    (external_dir / "stale").mkdir(parents=True)
    (external_dir / "active").mkdir(parents=True)
    ...
    assert not (external_dir / "stale").exists()
